...
constant escaped_chars = "tnr\\\""
function generate_char_map()
    sequence s
    s = repeat(0, 256)
    for i = 0 to 255 do
	s[i+1] = {i}
    end for
    for j = 1 to length(escape_chars) do
        s[(escape_chars[j])+1] = "\\" & escaped_chars[j]
    end for
    return s
end function

constant escape_map = generate_char_map()
    

function esc_char(integer a)
-- show escaped characters
	return escape_map[a+1]
end function
...
global constant PRETTY_DEFAULT = {1, 2, 1, 78, "%d", "%.10g", 32, 127 - (platform() = LINUX), 1000000000, 1}
...
-- elements may be accessed using the following global constant~:
--
-- # ##DISPLAY_ASCII##
-- # ##INDENT##
-- # ##START_COLUMN##
