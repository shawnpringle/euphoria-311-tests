...
include misc.e
constant SLASH = (platform() = LINUX) * '/' + (platform() = WIN32) * '\\'
include pretty.e as pretty
include file.e as io

function magnitude(object s)
	atom m m = 0
	if atom(s) then
		if s < 0 then
			s = -s
...
-- default TF = 0
procedure test_failed(sequence name, object a, object b, integer TF)
...
global procedure set_accumulate_summary(integer accumulate)
...
-- TF defaults to 0
procedure record_result(integer success, sequence name, object a, object b, integer TF)
	test_count += 1
...
global procedure test_equal(sequence name, object expected, object outcome)
	integer success
	...
	if equal(expected, outcome ) then
		...
		success = magnitude( expected - outcome ) < 1e-9
	else
		success = 0
	end if
			
	record_result(success, name, expected, outcome, 0)
end procedure
...
global procedure test_not_equal(sequence name, object a, object b)
	...
	record_result(success, name, a, b, 0)
end procedure
...
global procedure test_true(sequence name, object outcome)
...
global procedure assert(object name, object outcome)
...
global procedure test_false(sequence name, object outcome)
...
for i = 3 to length(cmd) do
	...
	elsif match("-accumulate", cmd[i]) = 1 then
